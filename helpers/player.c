...
/*   Updated: 2024/07/10 20:12:33 by emyildir         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	return (entity == EMPTY_CHR || entity == COLLECTIBLE_CHR \
	|| entity == EXIT_CHR);
}
...
void	move(t_game	*game, int dir)
{
	...
	if (is_moveable(game->map->map[current.y + diff.y][current.x + diff.x]))
		...
		(t_point){.x = current.x + diff.x, .y = current.y + diff.y, .dir = game->player_point.dir});
	if (dir == MOVE_RIGHT)
		game->player_point.dir = RIGHT;
	else if(dir == MOVE_LEFT)
		game->player_point.dir = LEFT;
}
